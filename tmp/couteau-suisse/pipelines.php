...
if($id=="switchcase") $texte=str_replace(array("quote>"),array("blockquote>"),$texte);
if($id=="timezone")
...
if($id=="blocs") $texte=str_replace(array("code>", "quote>"),array("cadre>", "blockquote>"),$texte);
if($id=="class_spip")
		$texte=str_replace("@h3@","h".cs_niveau_intertitres(true),$texte);
if($id=="spip_cache"){$tmp_nb = intval(defined("_CACHE_PROFONDEUR_STOCKAGE") ? _CACHE_PROFONDEUR_STOCKAGE : 4);
		$texte = str_replace(array("@_CS_CACHE_NB@", "@_CS_CACHE_NBR@"), array("16^$tmp_nb = " . (16 ** $tmp_nb), 16 ** ($tmp_nb/2)), $texte);
	}
...
include_spip('outils/ecran_securite');
// rajeunissement pour SPIP3&4 (2e appel du pipeline)
if($flux["outil"]=="maj_auto" && isset($flux["texte"]) && strpos($flux["fichier_distant"],"action/charger_plugin.php")!==false)
	$flux["texte"] = str_replace(array("'icon'","include_spip('inc/install');","recuperer_page"), array("'logo'", "if(_request('cs_retour')) return array('nom'=>\$retour, 'suite'=>\$suite, 'fichier'=>\$fichier, 'tmp'=>\$status['tmpname']);\n\tinclude_spip('inc/install');", "cs_recuperer_page"), $flux["texte"]);
