...
/*
 * Squelette : ../plugins/auto/saisies/v6.2.0/prive/style_prive_plugin_saisies.html
 * Date :      Thu, 12 Feb 2026 02:06:02 GMT
 * Compile :   Thu, 19 Feb 2026 01:05:01 GMT
 * Boucles :   _saisies, _si_yaml
 */ 

function BOUCLE_saisieshtml_616760c42dc5bf3cfc1099f2d1a9d3e2(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['source'] = array(retablir_echappements_modeles(saisies_lister_disponibles('saisies','')));
	...
	$iter = Spip\Compilateur\Iterateur\Factory::create(
		"DATA",
		$command,
		array('../plugins/auto/saisies/v6.2.0/prive/style_prive_plugin_saisies.html','html_616760c42dc5bf3cfc1099f2d1a9d3e2','_saisies',17,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
' { ' .
(($t1 = strval(retablir_echappements_modeles(interdire_scripts(extraire_attribut(filtrer('image_graver', filtrer('image_reduire',safehtml((isset($Pile[$SP]['icone'])?$Pile[$SP]['icone']:(($Pile[0]['icone'] ?? null)))),'16')),'src')))))!=='' ?
...
');
	...
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_saisies @ ../plugins/auto/saisies/v6.2.0/prive/style_prive_plugin_saisies.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_si_yamlhtml_616760c42dc5bf3cfc1099f2d1a9d3e2(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {

	static $command = array();
	static $connect;
	$command['connect'] = $connect = '';
	if (!isset($si_init)) { $command['si'] = array(); $si_init = true; }
	$command['si'][] = retablir_echappements_modeles(interdire_scripts(filtre_info_plugin_dist("yaml", "est_actif")));
	...
	$iter = Spip\Compilateur\Iterateur\Factory::create(
		"CONDITION",
		$command,
		array('../plugins/auto/saisies/v6.2.0/prive/style_prive_plugin_saisies.html','html_616760c42dc5bf3cfc1099f2d1a9d3e2','_si_yaml',16,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
'
' .
BOUCLE_saisieshtml_616760c42dc5bf3cfc1099f2d1a9d3e2($Cache, $Pile, $doublons, $Numrows, $SP) .
'
');
	}
	$iter->free();
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_si_yaml @ ../plugins/auto/saisies/v6.2.0/prive/style_prive_plugin_saisies.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette ../plugins/auto/saisies/v6.2.0/prive/style_prive_plugin_saisies.html
// Temps de compilation total: 1.609 ms
//

function html_616760c42dc5bf3cfc1099f2d1a9d3e2($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
' .
BOUCLE_si_yamlhtml_616760c42dc5bf3cfc1099f2d1a9d3e2($Cache, $Pile, $doublons, $Numrows, $SP) .
'
...
.saisies-menu-onglets__item .saisies-menu-onglets__lien.actif {
	color: ' .
(($t1 = strval(retablir_echappements_modeles(interdire_scripts(filtrer('couleur_foncer',entites_html(table_valeur($Pile[0]??[], (string)'foncee', null),true),'0.1')))))!=='' ?
		('#' . $t1) :
		'') .
';
	border-bottom-color: ' .
...
.saisies-menu-onglets__lien.erreur:after {
	...
	height: 1em;
	background: url(' .
retablir_echappements_modeles(chemin_image((string)'erreur-16.png')) .
') no-repeat;
...
');
	...
	return analyse_resultat_skel('html_616760c42dc5bf3cfc1099f2d1a9d3e2', $Cache, $page, '../plugins/auto/saisies/v6.2.0/prive/style_prive_plugin_saisies.html');
}
