...
function BOUCLE_objetshtml_5980d19abce2a97a91ac40daaffb4b7a(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = Spip\Compilateur\Iterateur\Factory::create(
		"DATA",
		$command,
		array('../prive/themes/spip/lists.css.html','html_5980d19abce2a97a91ac40daaffb4b7a','_objets',196,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
'
' .
(($t1 = strval(retablir_echappements_modeles(chemin_image((string)(interdire_scripts(concat(safehtml(table_valeur($Pile[$SP]['valeur'], 'icone_objet')),'-24.png')))))))!=='' ?
		((	'.liste-objets.' .
...
function BOUCLE_objets2html_5980d19abce2a97a91ac40daaffb4b7a(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = Spip\Compilateur\Iterateur\Factory::create(
		"DATA",
		$command,
		array('../prive/themes/spip/lists.css.html','html_5980d19abce2a97a91ac40daaffb4b7a','_objets2',1004,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
' .item > a {
	...
	' .
(($t1 = strval(retablir_echappements_modeles(chemin_image((string)(	(interdire_scripts(safehtml(table_valeur($Pile[$SP]['valeur'], 'icone_objet')))) .
	'-16.png')))))!=='' ?
		('background-image: url(' . $t1 . ');') :
		'') .
'
...
function html_5980d19abce2a97a91ac40daaffb4b7a($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
' .
BOUCLE_objetshtml_5980d19abce2a97a91ac40daaffb4b7a($Cache, $Pile, $doublons, $Numrows, $SP) .
'
/* pour editer liens ? */
...
.item-tri:after {
	...
	mask-image: url("' .
retablir_echappements_modeles(chemin_image((string)'tri-sens-off-xx.svg')) .
'");
	opacity: 0.5;
}
...
.item-tri_asc:not(.item-tri_actif):focus:after {
	mask-image: url("' .
retablir_echappements_modeles(chemin_image((string)'tri-sens-asc-xx.svg')) .
'");
}
...
.item-tri_desc:not(.item-tri_actif):focus:after {
	mask-image: url("' .
retablir_echappements_modeles(chemin_image((string)'tri-sens-desc-xx.svg')) .
'");
}
...
' .
BOUCLE_objets2html_5980d19abce2a97a91ac40daaffb4b7a($Cache, $Pile, $doublons, $Numrows, $SP) .
'
...
');
	...
	return analyse_resultat_skel('html_5980d19abce2a97a91ac40daaffb4b7a', $Cache, $page, '../prive/themes/spip/lists.css.html');
}
