...
function mailsubscribers_verifier_args_action($action) {
	...
	if (
		...
		if ($arg !== $cle and is_null($id_mailsubscribinglists)) {
			...
			if (empty($subscription['id_mailsubscribinglist'])) {
	...
	return [$email, $id_mailsubscribinglists];
}
...
function mailsubscribers_obfusquer_mailsubscriber($id_mailsubscriber) {
	...
	if (
		$row
		and in_array($row['statut'], ['refuse', 'poubelle'])
		...
		objet_modifier(
			'mailsubscriber',
			$id_mailsubscriber,
			['email' => mailsubscribers_obfusquer_email($row['email'])]
		);
...
function mailsubscribers_compte_inscrits($liste, $statut = 'valide', $id_segment = 0) {
	...
	if (is_null($count) or isset($GLOBALS['mailsubscribers_recompte_inscrits'])) {
		$count = [];
		$rows = sql_allfetsel(
			'id_mailsubscribinglist,statut,id_segment,count(id_mailsubscriber) as n',
		...
		$ids = array_column($rows, 'id_mailsubscribinglist');
		$listes = sql_allfetsel(
			'id_mailsubscribinglist,identifiant',
...
function mailsubscribers_trouver_fonction_synchro($liste) {
	...
	if (function_exists($f = "mailsubscribers_synchro_list_newsletter_$f")) {
		return $f;
	}
	return '';
}
		...
		foreach ($subs as $sub) {
			if (isset($identifiants[$sub['id_mailsubscribinglist']])) {
				...
				if ($sub['statut'] == 'valide') {
					...
					$status = 'on';
					$infos['status'] = 'on';
				} elseif (in_array($sub['statut'], ['prepa', 'prop'])) {
					$status = 'pending';
					if ($infos['status'] == 'off') {
...
function mailsubscriber_declarer_informations_liees() {
	...
	if (is_null($declaration)) {
		...
		foreach ($langues as $langue) {
			$infos['lang']['options']['data'][$langue] = traduire_nom_langue($langue);
		}

		// des champs extras ?
		if (test_plugin_actif('cextras')) {
			$saisies_tables = pipeline('declarer_champs_extras', []);
		...
		foreach ($groupes as $groupe) {
			$infos['groupemots_' . $groupe['id_groupe']] = [
				'titre' => $groupe['titre'],
				'saisie' => 'mot',
				'options' => [
...
function mailsubscriber_recuperer_informations_liees($id_mailsubscriber, $email) {
	...
	if ($declaration = mailsubscriber_declarer_informations_liees()) {
		...
		foreach ($declaration as $nom => $champ) {
			...
			elseif (
				isset($champ['auto_mot']) and $champ['auto_mot']
 ...
 */
function mailsubscribers_listes($options = []) {
	...
	$rows = sql_allfetsel(
		'identifiant as id,titre,titre_public,descriptif,statut as status,adresse_envoi_nom as from_name,adresse_envoi_email as from_email,segments',
		'spip_mailsubscribinglists',
		$where,
		'',
...
function mailsubscribers_synchro_lists($listes = null) {
	if (is_null($listes)) {
		$listes = mailsubscribers_listes(['category' => 'newsletter']);
	}
...
function mailsubscribers_do_synchro_list($liste) {
	if ($f = mailsubscribers_trouver_fonction_synchro($liste)) {
		...
		if (
			...
			spip_log(
				"Synchronise liste $liste : abonnes mal formes en retour de la fonction $f",
 ...
 */
function mailsubscribers_synchronise_liste($liste, $abonnes, $options = []) {
	$listes = [$liste];
	...
	if (is_bool($options)) {
		$options = ['addonly' => $options];
	}
	$options = array_merge(['addonly' => false, 'graceful' => true], $options);
	...
	spip_log(
		"mailsubscribers_synchronise_liste $liste: " . count($subs) . ' abonnes deja dans la liste',
	...
	foreach ($subs as $sub) {
		...
		elseif (!$options['addonly']) {
			...
			$unsubscribe($sub['email'], ['listes' => $listes, 'notify' => false, 'remove' => true]);
			$i++;
		}
		if (time() >= $max_time) {
	...
	$emails_clairs = array_keys($abonnes_emails);
	$obfusques = array_combine(array_map('mailsubscribers_obfusquer_email', $emails_clairs), $emails_clairs);

	$start = 0;
	$limit = 5000;
	$nb_unsubs = 0;
	do {
		$unsubs = sql_allfetsel(
			'S.email',
			'spip_mailsubscribers as S JOIN spip_mailsubscriptions as L ON S.id_mailsubscriber=L.id_mailsubscriber',
			'L.id_mailsubscribinglist=' . intval($id_mailsubscribinglist) . ' AND L.id_segment=0 AND L.statut=' . sql_quote('refuse'),
			'',
			'L.id_mailsubscriber',
			"$start,$limit"
		);
		$start += $limit;
		$unsubs = array_column($unsubs, 'email');

		$is_in_clair = array_intersect($emails_clairs, $unsubs);
		$nb_unsubs += count($is_in_clair);
		foreach ($is_in_clair as $unsub_email) {
			unset($abonnes_emails[$unsub_email]);
		}
		unset($is_in_clair);

		$is_obfusque = array_intersect(array_keys($obfusques), $unsubs);
		$nb_unsubs += count($is_obfusque);
		foreach ($is_obfusque as $unsub_email) {
			unset($abonnes_emails[$obfusques[$unsub_email]]);
		}
		unset($is_obfusque);
	} while (count($unsubs));
	unset($emails_clairs, $obfusques, $unsubs);

	spip_log("mailsubscribers_synchronise_liste $liste: " . $nb_unsubs . ' ne veulent pas etre reabonnes a la liste', 'mailsubscribers' . _LOG_DEBUG);
...
/**
 * Detecter les boucles infernales d'inscription/desinscription par les robots qui lisent les mails des utilisateurs
 *
 * @param string $optin_logs
 * @param int $nb_subs_possibles
 * @param string? $date
 * @param string? $ip
 * @return bool
 */
function mailsubscriber_detecte_spambot($optin_logs, $nb_subs_possibles = 1, $date = null, $ip = null) {
	if (empty($optin_logs)) {
		return false;
	}
	$optin_logs = explode("\n", $optin_logs);
	$optin_logs = array_filter($optin_logs);

	if (!$date) {
		$date = date('Y-m-d');
	}
	// une boucle d'inscription = 1 mail de desinscription de tout + 1 mail par liste d'attente + 1 mail par liste d'inscription
	$nb_events_par_boucle = 2 * max($nb_subs_possibles, 1) + 1;

	$logs_today = mailsubscriber_filter_logs($optin_logs, $date);
	if (count($logs_today) >= 4 * $nb_events_par_boucle) {
		spip_log('mailsubscriber_detecte_spambot: detection spambot sur inscription ' . count($logs_today) . " events pour $date", 'mailsubscribers' . _LOG_INFO_IMPORTANTE);
		return true;
	}

	if (!$ip and PHP_SAPI !== 'cli') {
		$ip = $GLOBALS['ip'];
	}
	if (!empty($ip)) {
		$logs_today_ip = mailsubscriber_filter_logs($logs_today, $ip);
		if (count($logs_today_ip) >= 2 * $nb_events_par_boucle) {
			spip_log('mailsubscriber_detecte_spambot: detection spambot sur inscription ' . count($logs_today_ip) . " events pour $date + $ip", 'mailsubscribers' . _LOG_INFO_IMPORTANTE);
			return true;
		}
	}
	return false;
...
/**
 * Filtrer un tableau de logs optin par la présence d'une chaine
 * @param array $logs
 * @param string $chaine
 * @return array
 */
function mailsubscriber_filter_logs($logs, $chaine) {
	return array_filter($logs, function ($v) use ($chaine) {
 return stripos($v, $chaine) !== false;
	});
}
