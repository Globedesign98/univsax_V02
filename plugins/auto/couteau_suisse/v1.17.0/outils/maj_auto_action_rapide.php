...
if(version_compare(PHP_VERSION, '5.0.0', '>='))
	include_spip('outils/maj_auto_php5');
else {
	function cs_svn17($dir) { return false; } 
}
...
if(!function_exists('info_maj_spip')) {
	...
	function info_maj_spip(){
		...
		if (strncmp($maj, "<a", 2) === 0)
			$maj = extraire_attribut ($maj, 'title');
	...
	function compat_maj_spip($forcer = false) {
		include_spip('lib/maj_auto/distant_mise_a_jour');
		if(function_exists('genie_mise_a_jour_dist') && $forcer)
			return genie_mise_a_jour_dist(0);
	}
}
...
function info_maj_spip2(){
	...
	if($m = info_maj_spip_ext($m1, $m2, $m3)) {
		if($maj && $res) $m[$maj] = $res;
		$res = implode('<br/>', $m); 
	}
	if(!strlen($res)) return $res;
	$res = str_replace('|', '<br/>', $res); // nouveauté SPIP v4 ?
	// liens morts
	$res = preg_replace(',\[([^[]+)->\],', '$1', $res);
	return $tmp	. propre("\n|{{{$res}}}|")
		. (preg_match(",$m1\.$m2\.\d+,",$res) ? '<p>' . _T('couteau:maj_spip') . '</p>' : '')
		. '</fieldset>';
}
...
function maj_auto_action_rapide($actif) {
	...
	foreach ($plugins as $p) {
		...
		$dir = $extension ? _DIR_PLUGINS_DIST : _DIR_PLUGINS;
		$stop = time() - $time > $timeout;
		$infos = plugin_get_infos_maj($p, $stop, $dir);
		if(!isset($infos['nom']) || !isset($infos['prefix']))
			continue;
		$nom = trim($infos['nom']);
		if(!defined('_SPIP30000') && strtoupper($infos['necessite'][0]['id']) == 'SPIP')
			array_shift($infos['necessite']);
		$cfg = $maj_lib = $checked = '';
	...
	$html1 = "\n<div class='padd' id='maj_auto_div'>$html1<fieldset><legend class='padd'>"
		...
		. propre(
			(count($html_actifs)? "\n|{{" . _T('couteau:plug_actifs') . "<span id='maj_1'" . $sep . implode("\n", $html_actifs) . "\n" : '')
			. (count($html_extensions)? "\n|{{" . _T(defined('_SPIP30000')?'plugins_liste_dist':'plugins_liste_extensions') . "<span id='maj_2'". $sep . implode("\n", $html_extensions) . "\n" : '')
			. (count($html_inactifs)? "\n|{{" . _T('couteau:plug_inactifs') . "<span id='maj_3'". $sep . implode("\n", $html_inactifs) . "\n" : '')
		  )
...
function maj_auto_rev_distante($url, $timeout = false, $pattern = NULL, $lastmodified = 0, $force = false) {
	...
	$url_md5 = md5($url);
	if(!isset($maj_[$url_md5])) $maj_[$url_md5] = array(0, false);
	$maj = &$maj_[$url_md5];
	...
	elseif($timeout)
		...
		$distant = $maj[1] = ($pattern !== NULL)
			? (($distant = cs_recuperer_page($url))
				? (preg_match($pattern, $distant, $regs) ? $regs[1] : '-2')
				: '-1')
			: strlen(cs_recuperer_page($url, false, true, 0));
		$maj[0] = time();
...
function plugin_get_infos_maj($p, $timeout = false, $DIR_PLUGINS = _DIR_PLUGINS) {
	...
	$fichier_lu = lire_fichier($svn_rev = $DIR_PLUGINS . $p . '/svn.revision', $svn);
	$lastmodified = @file_exists($svn_rev) ? @filemtime($svn_rev) : 0;
	if($fichier_lu && preg_match(',<origine>(.+)</origine>,', $svn, $regs)) {
	...
	if($infos['svn'] = is_array($rev_local) || $rev_local < 0) {
		...
		// prise en compte d'un demenagement de la Zone...
		$url_origine = preg_replace(',/_plugins_/_(?:stable|dev|test)_/,', '/_plugins_/', $url_origine);
	}
	// URL https:// inattendu
	if(strncmp($url_origine, 'https://', 8) !== 0)
		$url_origine = '';
...
function maj_auto_svp_query($dir, &$infos) {
	...
	if($x = sql_fetsel('id_paquet,id_plugin,version,nom_archive', 'spip_paquets', 'src_archive=' . _q($dir)))
		...
		if(strlen($y['nom_archive']) && intval(maj_auto_rev_distante($f = $y['url_archives'].'/'.$y['nom_archive'])))
			$infos['zip_trac'] = $f;
...
function maj_auto_svp_maj_plugin($ids_paquet = array()) {
	...
	if(!count($messages['decideur_erreurs'])) {
		...
		$actions = defined('_SPIP30000') ? decoder_contexte_ajax(_request('_todo'), 'svp_todo') : false;
		if($actions === false) $actions = @unserialize(_request('_todo'));
		$rejets = is_array($actions) ? array_diff(array_keys($actions0), array_keys($actions)) : array();
		if(count($rejets)) {
			// probablement une action de reparation ou de MAJ de release sans changement de version
			// dans ce cas, on remplace simplement les anciens fichiers du plugin 
			if((include_spip('action/charger_plugin') OR include_spip('lib/maj_auto/distant_action_charger_plugin'))
				...
				// ici : methode SPIP2 qui fonctionne sous SPIP3&4
				// (prise en compte de fichiers fantomes restes indument apres mise à jour vers SPIP3)
...
function maj_auto_maj_auto_forcer_action() {
	...
	if(function_exists('compat_maj_spip'))
		compat_maj_spip(true); // pour SPIP < 2.1
	elseif($cron = charger_fonction('mise_a_jour', 'genie'))
		$cron(0);
...
function version_svn_courante2($dir) {
	...
	if(!$db) {
		...
		if(@file_exists($dir.'/.svn/entries'))
			return version_svn_courante($dir);
		// trunk et extensions
		$db = @file_exists($dir2 = realpath($dir . '/../.svn/wc.db'));
		if(!$db) {
			// branches
			$db = @file_exists($dir2 = realpath($dir . '/../../.svn/wc.db'));
			if($db)
				$b = basename(dirname($dir)) . '/' . basename($dir);
		} else
			$b = basename($dir);
	} else
		$b = '';
