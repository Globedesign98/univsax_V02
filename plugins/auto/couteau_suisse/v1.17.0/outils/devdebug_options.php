...
function devdebug_charger_debug() {
	...
	if (_DEVDEBUG_MODE == 1) {
		...
		$lang = (isset($GLOBALS['spip_lang']) && in_array($GLOBALS['spip_lang'], $devdebug_langues_phpdoc)) ? $GLOBALS['spip_lang'] : 'fr';
		devdebug_ini_set('docref_root', "http://www.php.net/manual/" . $lang . "/");
		devdebug_ini_set('docref_ext', '.php');
		// On lance le php error tracking quoiqu'il arrive
		devdebug_ini_set('track_errors', 1);
		// On evite d'afficher les erreurs repetees
		devdebug_ini_set('ignore_repeated_errors', 1);
		...
		if (defined('_DEVDEBUG_NIVEAU')) switch (_DEVDEBUG_NIVEAU) {
			case 'warning':
				$niveau = "E_ALL ^ E_NOTICE";
				break;
			case 'error':
				$niveau = "E_ALL ^ (E_NOTICE | E_WARNING)";
				break;
			case 'strict':
				$niveau = "-1";
				break;
			case 'all':
				$niveau = "E_ALL | E_DEPRECATED";
				break;
			case 'user':
				$niveau = "E_USER_NOTICE | E_USER_WARNING | E_USER_ERROR";
				break;
			case 'notice':
			default:
				$niveau = "E_ALL";
		...
		if (defined('_DEVDEBUG_ESPACE')) switch (_DEVDEBUG_ESPACE) {
			case 'public':
				if (!$prive) {
					devdebug_ini_set('display_errors', 1);
					eval("error_reporting($niveau);");
				}
				else devdebug_ini_set('display_errors', 0);
				break;
			case 'prive':
				if ($prive) {
					devdebug_ini_set('display_errors', 1);
	...
	elseif (_DEVDEBUG_MODE == 0) devdebug_ini_set('display_errors', 0);
}
...
function exec_debug() {
	...
	if ($connect_statut != "0minirezo" || !autoriser('configurer', 'configuration')) {
		echo minipres();
		exit;
	}
...
function devdebug_ini_set($option, $value) {
