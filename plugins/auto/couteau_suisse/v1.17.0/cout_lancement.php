...
function cs_initialisation($forcer = false, $init_includes = true) {
	...
	if(test_espace_prive() && defined('_DIR_PLUGIN_COUTEAU_SUISSE') && isset($GLOBALS['meta']['plugin'])) {
		...
		$vNew = ($liste && isset($liste['COUTEAU_SUISSE'])) ? $liste['COUTEAU_SUISSE']['version'] : '';
	...
	if ($log)
		cs_log("{$rand}cs_initialisation($forcer) $vOld => $vNew : Passage #$deja_passe_ici");
	...
	$test = 0;
	foreach ($metas_outils as $nom => $o) {
		// exception pour maj_auto, afin de faire fonctionner la MAJ du CS en un clic pour SPIP>=3
		$maj_ok = (isset($o['actif']) && $o['actif']) || ($nom == 'maj_auto');
		if ($maj_ok && isset($o['maj_distant']) && $o['maj_distant']) {
...
// array_map recursif
function cs_array_map($arr, $function) {
	$result = array();
	foreach ($arr as $key => $val)
		$result[$key] = is_array($val) ? cs_array_map($val, $function) : $function($val);
	return $result;
}

// evite les transformations typo dans les balises $balises
// par exemple pour <html>, <pre>, <cadre>, <code>, <frame>, <script>, <style> (balises par defaut) + <cite> et <acronym>, $balises = '|cite|acronym'
...
// si $balises == '' alors la protection est par defaut est $GLOBALS['cs_echapper'] (html|pre|code|cadre|frame|script|style + balises d'autres plugins)
// si $balises == false alors le texte est utilise tel quel, pas d'echappement
// si $balise[0] == '|' alors la protection est ajoutee aux balises par defaut $GLOBALS['cs_echapper']
function cs_echappe_balises($balises, $fonction, $texte, $arg = NULL) {
	static $balises_defaut;
	if (!strlen($texte ?? ''))
		return '';
	...
	if(!isset($balises_defaut)) {
		$GLOBALS['cs_echapper'] = isset($GLOBALS['cs_echapper'])
			? cs_array_map($GLOBALS['cs_echapper'], function($item) { return strpos($item, '|')===false ? $item : explode('|', $item); })
			: array();
		$balises_defaut = array();
		array_walk_recursive($GLOBALS['cs_echapper'], function($item) use (&$balises_defaut) { $balises_defaut[] = $item; });
		$balises_defaut = implode('|', array_unique($balises_defaut));
	}
	...
	// protection du texte
	if ($balises !== false) {
		if (!strlen($balises)) $balises = $balises_defaut;
			elseif ($balises[0]=='|') $balises = $balises_defaut . $balises;
...
function cs_ecrire_config($regexpr, $replace, $ajout_sinon = '') {
	if (!$regexpr)
		return;
	if (!is_array($regexpr))
		$regexpr = array($regexpr);
	if (!is_array($replace))
		$replace = array($replace);
	if (!is_array($ajout_sinon))
		$ajout_sinon = array($ajout_sinon);
	if (count($regexpr) != count($replace)) {
		if (defined('_LOG_CS'))
			cs_log("ERREUR : cs_ecrire_config() comporte 2 tableaux d'inegale longueur !");
		return;
	}
	...
	if ($fo) {
		if (lire_fichier($fo, $t) && strlen($t)) {
			foreach (array_keys($regexpr) as $i) {
				if (preg_match($regexpr[$i], $t))
					$t = preg_replace($regexpr[$i], $replace[$i], $t, 1);
				else if (isset($ajout_sinon[$i])) 
					$t = $t . "\n" . $ajout_sinon[$i]; // sequence de fin de php ? > non trouvee par exemple => ajout en fin de fichier
			}
	...
	// creation du fichier car il n'existe pas
	if (!count($ajout_sinon))
		return;
...
function cs_lire_data_outil($outil, $casier_ = '') {
	static $datas = array();
	...
	// outil pas encore installe ?
	include_spip('outils/' . $outil);
	if ((function_exists($f = $outil . '_installe') || function_exists($f = $f . '_dist'))
			&& ($tmp = $f())
			&& (($casier_ && isset($tmp[$ok = $casier_])) || (!$casier_ && (isset($tmp[$ok = 0]) || isset($tmp[$ok = $outil]))))
		 )
		return $tmp[$ok];
	return NULL;
}
