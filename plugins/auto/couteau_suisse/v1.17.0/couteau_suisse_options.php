...
if (defined('_SPIP_VERSION_ID') && _SPIP_VERSION_ID >= 40408) {
	@define('_SPIP40408', 1);
	@define('_SPIP40200', 1);
	@define('_SPIP40100', 1);
	@define('_SPIP40000', 1);
	...
	@define('_SPIP19300', 1);
	@define('_SPIP19200', 1);
} elseif (defined('_SPIP_VERSION_ID') && _SPIP_VERSION_ID >= 40200) {
	@define('_SPIP40200', 1);
	@define('_SPIP40100', 1);
	@define('_SPIP40000', 1);
	@define('_SPIP30200', 1);
	...
	@define('_SPIP19300', 1);
	@define('_SPIP19200', 1);
} elseif (defined('_SPIP_VERSION_ID') && _SPIP_VERSION_ID >= 40100) {
	@define('_SPIP40100', 1);
	@define('_SPIP40000', 1);
	@define('_SPIP30200', 1);
	@define('_SPIP30100', 1);
	...
	@define('_SPIP19300', 1);
	@define('_SPIP19200', 1);
} elseif (defined('_SPIP_VERSION_ID') && _SPIP_VERSION_ID >= 40000) {
	@define('_SPIP40000', 1);
	@define('_SPIP30200', 1);
	@define('_SPIP30100', 1);
	@define('_SPIP30000', 1);
...
} elseif ($GLOBALS['spip_version_code'] >= 22653) {
	@define('_SPIP30200', 1);
	@define('_SPIP30100', 1);
	@define('_SPIP30000', 1);
	@define('_SPIP20200', 1);
...
defined('_CS_HTMLA') || define('_CS_HTMLA', '<span class="csfoo htmla"></span>');
defined('_CS_HTMLB') || define('_CS_HTMLB', '<span class="csfoo htmlb"></span>');
defined('_CS_HTMLX') || define('_CS_HTMLX', '<span class="csfoo \w+"></span>');
// avec paragraphage intempestif :
defined('_CS_HTMLX2') || define('_CS_HTMLX2', '<p>(?:<br[^>]*>\s*)?<span class="csfoo \w+"></span></p>|<span class="csfoo \w+"></span>');
...
if (!defined('_DIR_PLUGIN_COUTEAU_SUISSE')) {
	...
	spip_log(' URI : ' . $_SERVER['REQUEST_URI'] . '. POST : ' . var_export($_POST, true));
	$zap = true;
}
...
if ($zap)
	...
	if (!(isset($GLOBALS['cs_spip_options']) && $GLOBALS['cs_spip_options']))
		$cs_exists = $cs_exists && file_exists($f_mso);
	if (!$cs_exists)
...
// Droits par defaut équivalents à 'configurer' les 'plugins', donc tous les administrateurs non restreints
...
// $opt doit représenter ici l'outil concerné : $outil
// Si $opt['autoriser'] (code PHP) n'est pas renseigné, ces droits natifs sont toujours accordés
function autoriser_outil_configurer_dist($faire, $type, $id, $qui, $opt) {
	if (!is_array($opt)) // éviter des erreurs inutiles ?
...
function cs_version_erreur(&$outil) {
	static $spip;
	if(!isset($spip))
		$spip = defined('_SPIP40000') ? _SPIP_VERSION_ID : $GLOBALS['spip_version_code'];
	return (isset($outil['version-min']) && version_compare($spip, $outil['version-min'], '<'))
		|| (isset($outil['version-max']) && version_compare($spip, $outil['version-max'], '>'));
}
