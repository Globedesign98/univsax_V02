...
if ($GLOBALS['spip_version_branche']>=4.2) {
    include_spip('formulaires/illustrer_document');
}


function formulaires_cisf_illustrer_document_charger_dist($id_document){
    
    if ($GLOBALS['spip_version_branche']>=4.2) {
        $valeurs = formulaires_illustrer_document_charger_dist($id_document);
        
    } else {
    
        include_spip('inc/documents');
	$valeurs = sql_fetsel('id_document,mode,id_vignette,extension','spip_documents','id_document='.intval($id_document));
	if (!$valeurs OR in_array($valeurs['mode'], array('logoon', 'logooff')))
...
function formulaires_cisf_illustrer_document_verifier_dist($id_document){
    
    if ($GLOBALS['spip_version_branche']>=4.2) {
	return formulaires_illustrer_document_verifier_dist($id_document);
...
function formulaires_cisf_illustrer_document_traiter_dist($id_document){
    
    if ($GLOBALS['spip_version_branche']>=4.2) {
        $res = formulaires_illustrer_document_traiter_dist($id_document);
...
//--------- Debut ajout CI ---------
    ...
    $res['redirect'] = generer_url_public("cisf_doc_modifier", "id_article=$id_article&show_docs=$id_document&id_document=$id_document");
//--------- Fin ajout CI ---------
