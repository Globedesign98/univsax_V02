...
function migrer_pays_table($table_sql, $colonne) {
	...
	if (!$pays_fr) {
		...
		$pays = sql_allfetsel(['id_pays', 'nom'], 'spip_pays');
		// creer un tableau 'nom_fr' => 'id_pays'
		foreach ($pays as $pay) {
			$pays_fr[homogeneiser_nom_pays($pay['nom'])] = $pay['id_pays'];
		}
		unset($pays);
	}
	...
	$elements = sql_allfetsel([$_id_objet, $colonne], $table_sql, "$colonne != ''", '', $colonne);
	...
	foreach ($elements_groupes as $nom_fr => $ids) {
		...
		if (count($ids) !== 0) {
			if (!isset($pays_fr[$nom_fr])) {
				...
				sql_updateq($table_sql, [$colonne => $id_pays], sql_in($_id_objet, $ids));
...
function migrer_pays_table_et_bilan($table, $colonne, $locace = false) {
	// lancer la migration
	[$oks, $erreurs] = migrer_pays_table($table, $colonne);
